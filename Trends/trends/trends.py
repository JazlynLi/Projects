...
def make_tweet_fn(text, time, lat, lon):
    ...
    "*** YOUR CODE HERE ***"
    tweet_info = {'text': text, 'time': time, 'lat': lat, 'lon': lon}
    def get_tweet_info(key):
        return tweet_info[key]
    return get_tweet_info
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    x, y = apply_to_all(latitude, polygon), apply_to_all(longitude, polygon)
    cx6A, cy6A, area = 0, 0, 0
    for i in range(len(polygon) - 1):
        cx6A += ( x[i] + x[i+1] ) * ( x[i] * y[i+1] - x[i+1]*y[i] )
        cy6A += ( y[i] + y[i+1] ) * ( x[i] * y[i+1] - x[i+1]*y[i] )
...
def find_state_center(polygons):
    ...
    total_cx, total_cy, total_area = 0, 0, 0

    for centroid in centroids:
        lat, lon, area = centroid[0], centroid[1], centroid[2]
        total_cx += lat * area
        total_cy += lon * area
        total_area += area

    cx = total_cx / total_area
    cy = total_cy / total_area
...
def group_tweets_by_state(tweets):
    ...
    """
    "*** YOUR CODE HERE ***"
    def nearest_state_pair(tweet):
        distance = []
        current_min = ''
        for state in us_states:
            current_dist = [geo_distance(tweet_location(tweet), find_state_center(us_states[state]))]
            distance += [current_dist]
            if min(distance) == current_dist:
                current_min = state
        return [current_min, tweet]

    pairs = [nearest_state_pair(tweet) for tweet in tweets]
...
def average_sentiments(tweets_by_state):
    ...
    for state in tweets_by_state:
        sentiment = [analyze_tweet_sentiment(tweet) for tweet in tweets_by_state[state]]
        known_sentiment = [sentiment_value(s) for s in sentiment if has_sentiment(s)]
        if len(known_sentiment) > 0:
            avg_sentiment[state] = sum(known_sentiment) / len(known_sentiment)
