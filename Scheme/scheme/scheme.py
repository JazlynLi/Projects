...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if not scheme_symbolp(name):
            raise SchemeError("Bad define statement")
        value = Pair(target.second, vals.second)
        env.define(name, do_lambda_form(value, env))
        return name
    else:
...
def do_let_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    ###########################################################################
    # Process the bindings, so that if there were any repeated symbol, only the
    # last occurance of such symbol is preserved in the processed bindings.
    # eg. (let ((x 5) (y 2) (x 3)) x) should return 3

    # create a list of non-repeated parameters that has the correct order
    params = []
    for value in bindings:
        ...
        if param in params:
            params.remove(param)
        params.append(param)
    # create correct bindings without repeated symbols using params
    binds = nil
    i = len(bindings) - 1
    while i >= 0:
        curr = bindings[i][0]
        if curr in params:
            binds = Pair(bindings[i], binds)
            params.remove(curr)
        i -= 1
    ###########################################################################

    for value in binds:
        check_form(value, 2, 2)
        if not scheme_symbolp(value.first):
...
def check_formals(formals):
    ...
    """
    "*** YOUR CODE HERE ***"
    # if not isinstance(formals, Pair) and formals != nil:
    ...
    for param in formals:
        if scheme_symbolp(param):
            if param in symbols:
                raise SchemeError("Bad list, repeated symbol")
            symbols.append(param)
        else:
            raise SchemeError("Bad list, not a valid symbol")
