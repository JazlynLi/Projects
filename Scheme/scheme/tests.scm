...
(#t #t)
; expect Error: Cannot call True

(even? (* 2 4))
; expect True

; Q5
(define 61a (display 'cs ))
; expect cs61a

good morning
; expect Error

(define technicals '(compsci math physics))
(define courses (append technicals '(english)))
courses
; expect (compsci math physics english)
...
; Q7, Q8, Q9
(begin (display 'testing) (/ 1 0))
; expect testingError: division by zero

(define fn (lambda (x y) (display 'fall) (+ (* x 2) y)))
(fn 1000 14)
; expect fall2014
...
; Q13, Q14
(if (or #t (/ 1 0)) 'hella (zero? 3))
; expect hella

(if #f 'wrong (and 24 5))
; expect 5

(if #t (and 24 (/ 5 0)) 'wrong)
; expect Error: division by zero

(if #t (or 24 (/ 5 0)) 'wrong)
; expect 24
...
(let ((x 5) (y 3)) (average x y))
; expect 4

(let ((x 5) (x 3)) x)
; expect 3

(let ((x 5) (7 3)) (display 'hello))
; expect Error: not a valid symbol

; Q17
(define f (mu (x) (display 'forever) (+ x y)))
(define g (lambda (x y) (f (* x 2))))
(g 8 5)
; expect forever21
