...
##
##  Additional Primitives from MATH and TURTLE
##

from math import sqrt, sin, cos, tan, atan2, degrees, radians
...
@primitive("caddr")
def scheme_caddr(x):
    check_type(x, scheme_pairp, 0, 'caddr')
    return x.second.second.first
...
@primitive("atan")
def scheme_atan(val0, val1):
    return atan2(val0, val1)

@primitive("degrees")
def scheme_degrees(val):
    return degrees(val)

@primitive("radians")
def scheme_radians(val):
    return radians(val)

@primitive("bgcolor")
def tscheme_bgcolor(*args):
    _tscheme_prep()
    turtle.bgcolor(*args)
    return okay

@primitive("color2")
def tscheme_color2(r, g, b):
    _tscheme_prep()
