...
class Place:
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if self.ant is None:
                self.ant = insect
            elif self.ant.can_contain(insect) is True:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            "*** YOUR CODE HERE ***"
            if insect.name == 'Queen' and insect.is_impostor is False:
                return
...
class Ant(Insect):
    ...
    name = '' # an unspecified ant has no name
    damage = 0
    food_cost = 0
    blocks_path = True
    container = False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current is not hive:
            if len(current.bees) != 0 and self.min_range <= distance <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            ...
            print('{0} ran out of armor and exploded'.format(self))
            self.place.remove_insect(self)
...
class ScubaThrower(ThrowerAnt):
    """A ScubaThrower is a ThrowerAnt that is watersafe."""
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting == 0:
            # ready to eat!
            unlucky_bee = random_or_none(self.place.bees)
            if unlucky_bee is not None:
...
class QueenPlace:
    ...
    def __init__(self, colony_queen, ant_queen):
        ...
        self.current_place = ant_queen

    @property
    def bees(self):
        "*** YOUR CODE HERE ***"
        total_bees = self.current_place.bees + self.original_place.bees
        return total_bees
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        QueenAnt.num_constructed += 1
        if QueenAnt.num_constructed > 1:
            self.is_impostor = True
        else:
            self.is_impostor = False

    def ant_list(start_place, direction):
        """Return a list of ants that the queen can affect in
        DIRECTION (exit direction or entrance direction)."""

        ants = []
        next = lambda place, d: place.exit if d == 'exit' else place.entrance
        current_place = next(start_place, direction)
        while current_place:
            ant = current_place.ant
            if ant:
                ants.append(ant)
                # if ant is a bodyguard containing an ant
                if ant.container and ant.ant:    
                    ants.append(ant.ant)
            current_place = next(current_place, direction)
        return ants
    ...
    def action(self, colony):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Short-lived imposter!
        if self.is_impostor:
            self.armor = 0
            return 
        # Update queen's place
        colony.queen = QueenPlace(colony.queen, self.place)
        # Queen throws a leaf
        ThrowerAnt.action(self, colony)
        # all ants in front of and behind the queen
        ants = QueenAnt.ant_list(self.place, 'exit') + QueenAnt.ant_list(self.place, 'entrance')
        # check if the queen has a bodyguard
        queen_guard = self.place.ant
        if self.place.ant.container:
            ants.append(queen_guard)
        # double the damage
        for ant in ants:
...
def make_stun(action):
    ...
    """
    "*** YOUR CODE HERE ***"
    return lambda self: lambda colony: None
